...
-- Both operators (mom, farrel) have full CRUD on all operational tables.
-- Hard DELETE is denied everywhere (no DELETE policies created).
-- app_config: any authenticated user can SELECT; only farrel can UPDATE.
...
CREATE POLICY "operators_select_users" ON users
  ...
  USING (
    auth.uid() IN (SELECT user_id FROM app_config WHERE role IN ('mom', 'farrel'))
    AND deleted_at IS NULL
  );

CREATE POLICY "operators_insert_users" ON users
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() IN (SELECT user_id FROM app_config WHERE role IN ('mom', 'farrel')));

CREATE POLICY "operators_update_users" ON users
  FOR UPDATE TO authenticated
  USING  (auth.uid() IN (SELECT user_id FROM app_config WHERE role IN ('mom', 'farrel')))
  WITH CHECK (auth.uid() IN (SELECT user_id FROM app_config WHERE role IN ('mom', 'farrel')));
...
CREATE POLICY "farrel_update_config" ON app_config
  FOR UPDATE TO authenticated
  USING  (auth.uid() = (SELECT user_id FROM app_config WHERE role = 'farrel'))
  WITH CHECK (auth.uid() = (SELECT user_id FROM app_config WHERE role = 'farrel'));
...
CREATE POLICY "operators_select_photos" ON storage.objects
  ...
  USING (
    ...
    AND auth.uid() IN (SELECT user_id FROM app_config WHERE role IN ('mom', 'farrel'))
  );
